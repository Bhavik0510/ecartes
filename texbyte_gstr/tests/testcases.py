...
class GSTRTestcases(common.SingleTransactionCase):
    def setUp(self):
        ...
        self.fiscal_export = self.env['account.fiscal.position'].search([('name', '=', 'Export'), ('company_id', '=', self.env.company.id)], limit=1)
        ...
        self.currency_usd.active = True  # set currency active (avoid, UserError: You cannot validate an invoice with an inactive currency: USD)
