...
class GSTR1Report(models.TransientModel):
    ...
    date_from = fields.Date('Date From', default=date.today().replace(day=1) - relativedelta(months=1))
    date_to = fields.Date('Date To', default=date.today().replace(day=1) - relativedelta(days=1))
    ...
    def generate_hsn_report(self, wb1):
        ...
        # Error handling is not taken into consideraion
        self.ensure_one()
        # HSN is split into B2B and B2C now in GSTR1 offline tools
        b2b_invoices = (sorted_invoices + refund_invoices).filtered(lambda p: p.partner_id.vat)  #GST registered customers
        self.generate_hsn_report_common(wb1, 'hsn.b2b', b2b_invoices)
        b2c_invoices = (sorted_invoices + refund_invoices) - b2b_invoices
        self.generate_hsn_report_common(wb1, 'hsn.b2c', b2c_invoices)

    def generate_hsn_report_common(self, wb1, wsname, invoices):
        ws1 = wb1.add_sheet(wsname or 'hsn')
        fp = BytesIO()
        ...
        for invoice in invoices:     #Subtract refunded quantity/amount
            ...
            #Skip Roundoff/Discount/Extra Charges/Advance items and lines without product id
            for invoice_line in invoice.invoice_line_ids.filtered(lambda l: l.product_id and l.product_id.default_code not in ('ADVANCE','CHARGES','DISCOUNT')):
                prod_id = invoice_line.product_id
                hsn_code = prod_id.l10n_in_hsn_code and prod_id.l10n_in_hsn_code or '030289'    #Others
                hsn_name = prod_id.l10n_in_hsn_warning and prod_id.l10n_in_hsn_warning or 'Others'
                tax_rate = self.get_num(prod_id.taxes_id and prod_id.taxes_id[0].name.split('%')[0] or "0")  #consider first/main tax id
                line_uom = invoice_line.product_uom_id
                base_uom = prod_id.uom_id
                ...
                for tax_line in line_taxes['taxes']:
                    ...
                    elif 'cess' in tax_line['name'].lower(): #tax_obj.gst_type == 'cess': skip other taxes like TCS/TDS
                        cess_amount += tax_line['amount']

                if hsn_summary_data.get((hsn_code, base_uom, tax_rate)):      #Group by (hsn,uqc,rate) as same [4-digit] hsn can have different unit/rates. See https://tutorial.gst.gov.in/userguide/returns/Manual_Creation_of_Outward_Supplies_Return_in_GSTR-1A.htm#Table_12_HSN
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)][2] += line_qty
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)][3] += line_total_amount
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)][4] += line_amount
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)][5] += igst_amount
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)][6] += cgst_amount
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)][7] += sgst_amount
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)][8] += cess_amount
                else:
                    hsn_summary_data[(hsn_code, base_uom, tax_rate)] = [hsn_name, base_uom.l10n_in_code, line_qty, line_total_amount, line_amount,\
                                                              igst_amount, cgst_amount, sgst_amount, cess_amount]

        #Sort by (hsn,tax)
        for product_hsn_uom_tax, hsn_sum in sorted(hsn_summary_data.items(), key=lambda p:p[0]):
            row += 1
            ws1.write(row, col +  1, product_hsn_uom_tax[0], line_content_style)        #HSN code
            ws1.write(row, col +  2, hsn_sum[0], line_content_style)                    #Description
            ws1.write(row, col +  3, hsn_sum[1], line_content_style)                    #UoM GST code
            ws1.write(row, col +  4, hsn_sum[2], line_content_style)                    #Quantity in Base UoM
            ws1.write(row, col +  5, hsn_sum[3], line_content_style)                    #Amount
            ws1.write(row, col +  6, float(product_hsn_uom_tax[2]), line_content_style) #Tax rate
            ws1.write(row, col +  7, hsn_sum[4], line_content_style)                    #Taxable amount
            ws1.write(row, col +  8, hsn_sum[5], line_content_style)                    #IGST
            ws1.write(row, col +  9, hsn_sum[6], line_content_style)                    #CGST
            ws1.write(row, col + 10, hsn_sum[7], line_content_style)                    #SGST
            ws1.write(row, col + 11, hsn_sum[8], line_content_style)                    #Cess
    ...
    def generate_b2cs_report(self, wb1):
        ...
        #Sale to unregistered customers other than B2CL (unreg. intra-state for any amount + unreg. inter-state <= 2.5 lakh), deduct credit notes
        for invoice in (sorted_invoices + refund_invoices).filtered(lambda p: not p.partner_id.vat \
            ...
            for invoice_line in invoice.invoice_line_ids:
                price = invoice_line.price_unit * (1 - (invoice_line.discount or 0.0) / 100.0)
                line_taxes = invoice_line.tax_ids.compute_all(price, invoice.currency_id, invoice_line.quantity, invoice_line.product_id, invoice.partner_id)
                #_logger.info(line_taxes)
                line_taxes['total_excluded'] *= sign
                line_taxes['total_included'] *= sign
                for l in line_taxes['taxes']:
                ...
                for ln_tx in invoice_line.tax_ids: #.sorted(reverse=True):
                    if 'GST' in ln_tx.name:    # GST taxes are named 'IGST', 'SGST', 'CGST' etc
                        ...
                        if invoice_pos_tax_lines.get(PoS):
                            ...
                            invoice_pos_tax_lines[PoS][gst_tax_id] = {'name': ln_tx.name, 'base_amount': line_taxes['total_excluded'], 'cess_amount': 0}
                        invoice_pos_tax_lines[PoS][gst_tax_id]['cess_amount'] += sum(l['amount'] for l in line_taxes['taxes'] if 'cess' in l['name'].lower())
            ...
            for inv_line in invoice.invoice_line_ids:
                ...
                for ln_tx in inv_line.tax_ids: #.sorted(reverse=True):
                    if 'GST' in ln_tx.name:    # GST taxes are named 'IGST', 'SGST', 'CGST' etc
                        ...
                        # Add Cess amount if the Cess tax is also included in this group tax
                        grouped_tax_lines[gst_tax_id]['cess_amount'] += sum(l['amount'] for l in line_taxes['taxes'] if 'cess' in l['name'].lower())
    ...
    def gst_inv_type_from_l10n(self, invoice):
        inv_type_gst = inv_type_map[invoice.l10n_in_gst_treatment] if invoice.l10n_in_gst_treatment else "Regular"
    ...
    def format_place_of_supply(self, state_id):
        return state_id and state_id.l10n_in_tin and (state_id.l10n_in_tin + "-" + GST_POS.get(state_id.l10n_in_tin)) or ""
