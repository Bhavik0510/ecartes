...
class GSTR9Report(models.TransientModel):
    ...
    date_from = fields.Date('Date From')    #TODO: financial year start & end
    ...
    def outward_tax_payable_report(self, wb1):
        ...
        self.summarize_tax_values(b2c_invoices, b2c, lambda p: all(tax.name not in ['Exempt Sale', 'Nil Rated'] for tax in p.tax_ids))
        b2b_invoices = sorted_invoices.filtered(lambda p: p.partner_id.vat and p.move_type == 'out_invoice')  # GST registered customers
        self.summarize_tax_values(b2b_invoices, b2b, lambda p: all(tax.name not in ['Exempt Sale', 'Nil Rated'] for tax in p.tax_ids))
    ...
    def hsn_outward(self, wb1):
        ...
        for product_hsn, hsn_sum in sorted(hsn_summary_data.items(), key=lambda p: p[0].name):
            ...
            ws1.write(row, col + 3, product_hsn.uom_id.l10n_in_code.split('-')[0] if product_hsn.uom_id.l10n_in_code else "Not set", line_content_style) #UQC
        ...
        for invoice in invoice_list:
            ...
            for inv_line in invoice.invoice_line_ids.filtered(inv_line_filter_fn):  # Filter lines if necessary (e.g. goods/service)
                ...
                for tax in line_taxes['taxes']:
                    ...
                    elif 'cess' in tax['name'].lower(): #tax_obj.gst_type == 'cess': skip other taxes like TCS/TDS
