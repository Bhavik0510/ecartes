...
class GSTR3BReport(models.TransientModel):
    ...
    def generate_3b_report(self, wb1):
        ...
        for invoice in sorted_invoices + refund_invoices:     #Net payable taxable + tax, subtracting credit notes
            ...
            for invoice_line in invoice.invoice_line_ids:
                ...
                for tax_line in line_taxes['taxes']:
                    ...
                    if float_is_zero(tax_line['amount'], precision_digits=2):
                        exempted_nil = True
                ...
                #TODO: Vendor Bills with reverse charge doesn't have tax lines filled, so it must be calculated
                    ...
                    if invoice._is_reverse_charge_applicable():
                        ...
                        if exempted_nil:  #Zero GST taxes (Exempted, Nil). NOTE: Odoo 16 RC (reverse charge) adds two lines of tax negate each other, treat them as RC
                            ...
                            else:   #Refund, ITC reversed
                                itc_reversed_1['igst'] += igst_amount
                                itc_reversed_1['cgst'] += cgst_amount
                                itc_reversed_1['sgst'] += sgst_amount

                    non_gst_supplies_group_id = self.env.ref('l10n_in.non_gst_supplies_group').id
                    for line in invoice_line:
                        tax_ids = line.tax_ids.children_tax_ids or line.tax_ids
                        if non_gst_supplies_group_id in tax_ids.tax_group_id.ids and line.tax_ids.type_tax_use == 'purchase':
                            outward_non_gst['taxable_value'] += line_amount
                            outward_non_gst['igst'] += igst_amount
                            outward_non_gst['cgst'] += cgst_amount
                            outward_non_gst['sgst'] += sgst_amount
                            outward_non_gst['cess'] += cess_amount
        ...
        def prepare_eligible_itc(row):
            ...
            ws1.write(row+10, col+2, xlwt.Formula('SUM(B21:B25)-SUM(B27,B28)'), line_content_style)
            ...
            ws1.write(row+10, col+3, xlwt.Formula('SUM(C21:C25)-SUM(C27,C28)'), line_content_style)
            ...
            ws1.write(row+10, col+4, xlwt.Formula('SUM(D21:D25)-SUM(D27,D28)'), line_content_style)
            ...
            ws1.write(row+10, col+5, xlwt.Formula('SUM(E21:E25)-SUM(E27,E28)'), line_content_style)
