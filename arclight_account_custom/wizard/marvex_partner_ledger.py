...
class MarvexPartnerLedger(models.Model):
    ...
    def get_updated_ledger_lines(self):
        ...
        for current_date in all_dates:
            ...
            # # ------------------------------------------------------
            # # STEP 3: PROCESS OTHER ENTRIES (Credit Notes, Journal Entries)
            # # ------------------------------------------------------
            # # Only process non-invoice, non-payment entries
            #
            # other_lines = all_move_lines.filtered(
            #     lambda r: r.date == current_date and r.move_id.move_type not in [
            #         'out_invoice'] and r.move_id.journal_id.type not in ['bank', 'cash']
            # )
            #
            ...
            #     book = (
            #         "Credit Note" if move.move_type == "out_refund"
            #         else "Journal Entry"
            #     )
            #
            ...
            #
            # # ------------------------------------------------------
            # # STEP 4: PROCESS PAYMENTS ONLY IF NO CONTAINERS AND NO BULK PAYMENTS
            # # ------------------------------------------------------
            # bulk_payments = all_bulk_payments.filtered(lambda b: b.date == current_date)
            #
            # if not containers and not bulk_payments:
            #     payment_lines = all_move_lines.filtered(
            #         lambda r: r.date == current_date and r.move_id.journal_id.type in ['bank', 'cash']
            #     )
            #
            ...
            #         )
            #
            #         narration = payment.partner_bank_id.acc_number if payment and payment.partner_bank_id else move.journal_id.name
            #
            #         running_balance += line.debit - line.credit
            #
            #         debit_total += line.debit
