...
class RenewedEmployeeDocuments(models.TransientModel):
    _name = "renewed.documents"
    _description = "Renewed Employee Documents generate"
    ...
    new_attachment_ids = fields.Many2many('ir.attachment', string="Attachments",
    ...
    def renewed_method(self):
        ...
        brws_id = self.env['hr.employee.document'].browse(active_id)
        vals = {'doc_attachment_id': self.new_attachment_ids,
    ...
    def default_get(self, fields):
        result = super(RenewedEmployeeDocuments, self).default_get(fields)
        active_id = self.env.context.get('active_ids')
        brws_id = self.env['hr.employee.document'].browse(active_id)
        if active_id:
            result.update({
